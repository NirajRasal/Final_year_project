...
tickers = 'UNIONBANK.NS'

df = yf.download(tickers,start=start,end=end)
df.index = df.index.strftime('%Y-%m-%d')
df.to_csv('Data/Bank_data/UNIONBANK_5Y.csv')
#
...
def choose_dataset(stocks, stocks_data, allmodels):
    ...
    check = st.sidebar.checkbox("Hide", value=False, key='1')
...
def about_section():
    ...
    st.sidebar.markdown("Niraj Rasal,Vijay Chavan,Rushikesh Rathod")
    st.sidebar.markdown("and Atul Giramkar")
...
if __name__ == "__main__":
    ...
    if not check:
        ...
        forecast_check = st.sidebar.checkbox("See the results", value=True)
        about_section()
        if forecast_check:
            st.subheader("Days of Forecast:30")
            forecast = 30
